...
# EXERCISE ----------------------------------------------------------------
  ...
  intersect(names(df_lp), names(df_msd_tst))

  # Try joining the two data sets together
  df_lj<-left_join(df_msd_tst,df_lp)

  
  #how are the join fields different between the datasets?
  setdiff(df_local$mech_code,df_msd_tst$mech_code)
